...
// FUNCTIONS
function room_name_validator(value) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        // GET the endpoint
        const response = yield fetch(`/check/room-name/${value}`);
...
function createChat(room_name, max_participants, username, room_password) {
    if (room_name.length < 1 || room_name.length > 20) {
        console.error('Cannot create chat: Room name is missing or invalid');
        return;
    }
    if (!max_participants || !parseInt(max_participants.toString())) {
        console.error('Cannot create chat: Max participants is missing or cannot be converted to integer');
        return;
    }
    max_participants = parseInt(max_participants.toString());
    if (!username) {
        console.error('Cannot create chat: Username is missing');
        return;
    }
    if (max_participants < 1 || max_participants > 10) {
        console.error('Cannot create chat: Max participants is outside the 1-10 range');
        return;
    }
...
create_button.onclick = function () {
    // @ts-ignore
    const room_name = room_name_input === null || room_name_input === void 0 ? void 0 : room_name_input.value;
    // @ts-ignore
    const room_maxParticipants = (room_maxParticipants_input === null || room_maxParticipants_input === void 0 ? void 0 : room_maxParticipants_input.value) || 5;
    // @ts-ignore
    const room_password = room_password_input === null || room_password_input === void 0 ? void 0 : room_password_input.value;
    // @ts-ignore
    const username = (username_input === null || username_input === void 0 ? void 0 : username_input.value) || url_username;
    if (!room_name) {
        alert('Room name is missing! Please fill all required inputs');
        return;
    }
    if (!parseInt(room_maxParticipants)) {
        alert('Max participants cannot be parsed to an integer! Please input correctly formatted values');
        return;
    }
    const room_n_maxParticipants = parseInt(room_maxParticipants);
    if (room_n_maxParticipants < 1 || room_n_maxParticipants > 10) {
        alert('Max participants is outside the 1-10 range! Please input accepted values');
        return;
    }
    if (!username) {
