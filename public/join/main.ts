...
{ joinChat(url_roomID, url_roomPassword, url_username) }
    ...
    if (url_roomPassword) {
        ...
        // @ts-ignore
        if (url_roomPassword) room_password_validator(url_roomPassword).then(is_valid => { setValidityClass(room_password_input, is_valid) })
    }
    if (url_username) {
        // @ts-ignore
        username_input.value = url_username ? url_username : ''
        // @ts-ignore
        if (url_username) username_validator(url_username).then(is_valid => { setValidityClass(username_input, is_valid) })
    }
}
